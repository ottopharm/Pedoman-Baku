...
class SiteController extends Controller {
    ...
    public function actionSaveNewPassword() {
        ...
        $current_pwd = Yii::app()->session['loginSession']['password'];
        // $user = UserMaster::model()->findByPk($user_id);

        try {
            if (md5($_POST['old_password']) != $current_pwd) {
                $flagError++;
                $msg = 'Old Password did not match with existing data.';
            } else {
                $command = Yii::app()->db->createCommand();
                $command->update('UserMaster', array(
    ...
    public function actionDisplayDokPb($nodok) {
        $this->renderPartial('display_dok_pb', array(
            'nodok' => $nodok));
    }
